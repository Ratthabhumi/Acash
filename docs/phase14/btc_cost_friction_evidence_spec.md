...
| **Holding Cost / Drag** | Zero holding cost (unless margin borrowed). | **Contract/Rule-Dependent Funding Rate** (e.g. 8-hour schedule on Binance USDⓈ-M; funding cash flows must be simulated separately). | Funding cash flow must be simulated separately. |
...
| **Liquidity & Depth** | Centralized spot order book; capital-constrained. | Speculative derivative order book; higher leverage. | Qualitative depth disparity (regime & venue dependent). |
...
- **Current Evidence Baseline:**
  - `UNRESOLVED — REQUIRES BID/ASK DATA`. No empirical top-of-book quote recording has been acquired or frozen for ACASH.
  - Any numerical spread value mentioned in research planning (such as 0.5 bps normal or 10.0 bps stress) is an non-binding heuristic template (`ILLUSTRATIVE / NOT FROZEN`), never an empirical finding.
...
For future empirical research, candidate strategies may evaluate market capacity using explicit volume participation ratios rather than assuming infinite depth:

### 7.1 Research Capacity Buckets (Illustrative / Non-Mandatory)
Future research candidates may evaluate illustrative parameterized order notional buckets (`ILLUSTRATIVE / NOT FROZEN — candidate evaluation templates`):
- **Bucket 1 (Micro / Test):** \$1,000 USD notional (trivial illustrative size).
- **Bucket 2 (Small Quantitative):** \$10,000 USD notional (`ILLUSTRATIVE`).
- **Bucket 3 (Medium Strategy):** \$50,000 USD notional (`ILLUSTRATIVE`).
- **Bucket 4 (Capacity Stress):** \$250,000 USD notional (`ILLUSTRATIVE`).

### 7.2 Volume Participation Ceiling (Illustrative Template)
- In any 1-minute bar window, candidate execution models may parameterize an illustrative maximum participation ratio $\alpha_{\text{max}}$ of total bar volume $V_{\text{bar}}$ (e.g. illustrative candidate range $\alpha_{\text{max}} = 0.01 \text{ to } 0.02$, `ILLUSTRATIVE / NOT FROZEN`):
  $$\frac{Q_{\text{order}}}{V_{\text{bar}}} \le \alpha_{\text{max}}$$
- Orders exceeding the candidate ceiling trigger simulated partial fills or multi-bar execution schedules in research tests.
...
1. **Binance Spot Trading Fee Schedule:**
   ...
   - Supported Claims: Regular User (VIP 0) standard fee is 0.1000% Maker and 0.1000% Taker; 25% discount when paying with BNB (0.0750%/0.0750%). Account/region/product dependent.
2. **Binance USDⓈ-M Futures Fee Schedule:**
   - URL: `https://www.binance.com/en/fee/futureFee`
   - Access Date: 2026-09-13 UTC
   - Supported Claims: Regular User (VIP 0) standard fee is 0.0200% Maker and 0.0500% Taker; 10% discount when paying with BNB (0.0180%/0.0450%); standard 8-hour funding intervals (contract/rule dependent).
...
- Mathematical Authority: PLANNING / NON-GOVERNING SPEC (Aligned with `src/acash/backtest/schema.py`)
...
- Methodological Caveats: Numerical thresholds for spread, latency, notional capacity, and market impact are explicitly marked `UNRESOLVED` or `ILLUSTRATIVE / NOT FROZEN` pending empirical high-frequency recordings.
