# MEC-0017 / HYP_007: Scientific Strategy Preregistration
...
[CANONICAL_COMMIT_LINEAGE: 9195caa8a9f1849f93fbfa806184e01ac99e4e8e]
...
[M1_SAMPLE: 2021-07-01 THROUGH 2024-04-30]
[M2_SAMPLE: 2024-05-01 ONWARD (LOCKED)]
[M3_SAMPLE: PROSPECTIVE_ONLY]
[CAPITAL: $0.00 / NO_REAL_ORDERS=true]
```

- **Document ID:** `docs/research/MEC-0017-HYP-007-strategy-preregistration.md`
...
> **HYP_007:** In the post-transition United States equity market (SPY), intraday price extensions beyond a 14-session trailing same-minute Noise Area anchored by dividend-adjusted prior close and confirmed by cumulative regular-session VWAP generate positive net economic returns after deducting realistic broker commissions, Section 31 and FINRA regulatory fees, quoted bid-ask spread crossing, and adverse execution slippage under direct-SIP historical data infrastructure ($K = 1$).

### 1.2 Motivation & Direct-SIP Lineage
HYP_006 established that pre-2021 historical quote records carry legacy `?` condition codes lacking granular CTA metadata.
HYP_007 replicates the exact scientific mechanism over the verified direct-SIP sample (`2021-07-01` through `2024-04-30`, ~2.83 years, ~710 trading sessions) where free, consolidated SIP bars and historical quotes are captured directly by Alpaca from CTA/UTP with verified official condition codes (`R`).

---

## 2. Partition Architecture & Governance

- **M1 (Replication Sample):** `2021-07-01` through `2024-04-30`.
  - Role: `PUBLICATION_EXPOSED_DIRECT_SIP_REPLICATION_SAMPLE`.
  - Start Date Provenance: Earliest conservative quarter boundary following direct-SIP transition (April 2021), providing a clean two-month operational separation buffer. Selected solely for data provenance, NOT strategy performance.
...
- **Anti-Contamination Rule:** Neither M1 nor M2 is described as pristine out-of-sample data.
...
- **Specification:** `MEC_0017_NOISE_AREA_INTRADAY_MOMENTUM_BASELINE`.
- **Prohibitions:**
  - Zero lookback searches ($14$ days strictly frozen).
  - Zero threshold or noise multiplier tuning ($1.0$ strictly frozen).
  - Zero long-only or short-only parameter search (symmetric trading).
...
- **Missing Bar Policy:** `FAIL_CLOSED_SESSION_EXCLUSION`. Any missing minute in a standard 390-minute session excludes the entire session.
...
### 7.2 Quote Validity & Condition Policy
- Authoritative mapping bound to Alpaca Tape B (`GET /v2/stocks/meta/conditions/quote?tape=B`).
- Acceptable condition codes: `{'R'}` (Regular, Two-Sided Open Quote).
...
### 7.3 End-of-Day (EOD) Execution Contract
...
- **Return Definition:** 15 historical daily simple close-to-close returns of unadjusted closes.
- **Parameters:**
  ...
  - Shares: Nearest integer rounding (`round(..., 0)`).
