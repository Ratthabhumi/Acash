...
const FeedHealthBadge: React.FC<FeedHealthBadgeProps> = ({ health }) => {
  const cfg: Record<FeedHealth, { label: string; cls: string; Icon: React.FC<{ className?: string }> }> = {
    ...
    UNKNOWN:      { label: 'Feed UNKNOWN',      cls: 'bg-surface-muted text-secondary border-default', Icon: Activity },
  };
...
export const SlotCard: React.FC<SlotCardProps> = ({ slot, rank }) => {
  ...
  return (
    <div
      className={`bg-surface rounded-xl border shadow-sm transition-colors flex flex-col gap-4 p-4
        ${isHalted ? 'border-rose-300 dark:border-rose-800' : 'border-default'}`}
      aria-label={`Strategy slot ${slotId}: ${strategyName}`}
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-accent text-white font-mono font-bold text-sm shrink-0">
            {slotId}
          </div>
          <div className="min-w-0">
            ...
            <div className="text-[11px] text-secondary font-mono truncate mt-0.5">
      ...
      {/* Unassigned notice */}
      {isUnassigned && (
        <div className="flex items-start gap-2 p-2.5 rounded-lg bg-surface-muted border border-default text-xs text-secondary">
          <Info className="w-3.5 h-3.5 shrink-0 mt-0.5" />
          <div>
            <div className="font-semibold text-primary mb-0.5">Strategy Not Assigned</div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-surface-muted rounded-lg p-3 border border-subtle">
          <div className="text-[10px] font-mono text-secondary uppercase tracking-wider mb-1">Virtual NAV</div>
        ...
        <div className={`rounded-lg p-3 border ${pnlPositive
          ...
          <div className="text-[10px] font-mono text-secondary uppercase tracking-wider mb-1">Virtual PnL</div>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[11px] font-mono">
        {[
          ...
          <div key={label} className="flex justify-between items-center py-0.5 border-b border-subtle">
...
const GlobalStatusBar: React.FC<GlobalStatusBarProps> = ({ global, fetchedAt, isMock }) => (
  <div className="bg-surface rounded-xl border border-default shadow-sm p-4 space-y-3">
    <div className="flex flex-wrap items-center justify-between gap-2">
      <div className="flex items-center gap-2">
        <Activity className="w-4 h-4 text-muted" />
        <span className="text-sm font-semibold text-primary">{global.label}</span>
        {isMock && (
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 text-[11px] font-mono">
      {[
        ...
        <div key={label} className="flex flex-col gap-0.5">
          ...
          <span className={`font-medium ${warn ? 'text-rose-600 dark:text-rose-400' : 'text-primary'}`}>
            {value}
          </span>
        </div>
      ))}
...
const Leaderboard: React.FC<LeaderboardProps> = ({ tournament }) => {
  ...
  return (
    <div className="bg-surface rounded-xl border border-default shadow-sm p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className={`ml-auto px-2 py-0.5 rounded text-[10px] font-mono font-medium border
          ${leaderboard.comparisonAvailability === 'INSUFFICIENT_SAMPLE'
            ? 'bg-surface-muted text-secondary border-default'
      ...
      <div className="text-[11px] text-secondary font-mono mb-3 p-2 bg-surface-muted rounded border border-subtle">
        <Info className="w-3 h-3 inline mr-1" />
        {leaderboard.comparisonNote}
      </div>

      {leaderboard.rankedSlots.length === 0 ? (
        <div className="text-center text-xs text-muted font-mono py-6">
...
<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <h2 className="text-xs font-mono font-semibold uppercase tracking-wider text-secondary mb-3">
      ...
      <div className="bg-surface-muted rounded-xl border border-default p-4">
        <div className="flex items-center gap-2 mb-2">
          <Lock className="w-3.5 h-3.5 text-muted" />
          <span className="text-[11px] font-mono font-semibold text-secondary uppercase tracking-wider">
            Architecture Invariants
          </span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 text-[10px] font-mono text-secondary">
          {[
