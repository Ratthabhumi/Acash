"""HYP_010 R2 tiny provider probe: SPY/VEU/AGG/BIL 1Day SIP split+raw, 2017-01-03..06.
...
# Inclusive scientific window mapped through the already-audited UTC-bounds
# helper (start-of-day .. end-of-day), so the final session's bar is included.
PROBE_START, PROBE_END = inclusive_date_window_to_utc_bounds(
    date(2017, 1, 3), date(2017, 1, 6)
)
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="HYP_010 tiny provider probe.")
    parser.add_argument("--execute-network", action="store_true", default=False)
    parser.add_argument("--out", default="data/hyp_010/probe_evidence.json")
    ...
    print("=== HYP_010 R2 TINY PROVIDER PROBE ===")
    ...
    _ = NyseCa1Calendar()
    client = HYP010AlpacaClient(http_attempt_listener=_count)
    ...
    for symbol in SYMBOLS:
        ...
        for adjustment in (PriceAdjustment.SPLIT, PriceAdjustment.RAW):
            ...
            except (AlpacaCredentialError, DataContractError) as exc:
                record["adjustments"][adjustment.value] = {
                    "status": type(exc).__name__,
                    "detail": str(exc)[:300],
                }
                failed = True
        # alignment check on success
    ...
    evidence["verdict"] = (
        "PROVIDER_PROBE_PASS" if not failed else "PROVIDER_PROBE_BLOCKED"
    )
