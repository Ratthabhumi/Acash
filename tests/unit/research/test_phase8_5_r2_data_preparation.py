...
def test_gate1_and_2_instrument_frequency_and_bar_count() -> None:
    """Gates 1 & 2: Verify EURUSD instrument, M5 frequency, and T >= 5,000 usable bars."""
...
def test_gate3_and_4_monotonicity_and_zero_duplicates() -> None:
    """Gates 3 & 4: Verify strictly monotonic timestamps and zero duplicate timestamps."""
    parquet_path = Path("data/parquet/research/EURUSD_M5_canonical.parquet")
    table = pq.read_table(parquet_path)
...
def test_gate5_ohlc_structural_integrity() -> None:
    """Gate 5: Verify OHLC structural invariants (H >= L, H >= O, H >= C, L <= O, L <= C, prices > 0)."""
    parquet_path = Path("data/parquet/research/EURUSD_M5_canonical.parquet")
    table = pq.read_table(parquet_path)
...
def test_gate6_gap_detection_and_classification() -> None:
    ...
    manifest_path = Path("data/manifests/research/manifest-EURUSD_M5_canonical.json")
...
def test_gate8_provenance_and_ledger() -> None:
    """Gate 8: Verify provenance record in JSONL ledger and batch manifest."""
...
def test_gate9_and_10_digests_and_manifest() -> None:
    """Gates 9 & 10: Verify content digests and dataset manifest consistency."""
    manifest_path = Path("data/manifests/research/manifest-EURUSD_M5_canonical.json")
    with open(manifest_path, "r", encoding="utf-8") as f:
        manifest = json.load(f)
...
def test_gate11_feature_non_anticipation() -> None:
    """Gate 11: Verify feature construction cannot use future observations."""
    parquet_path = Path("data/parquet/research/EURUSD_M5_canonical.parquet")
    table = pq.read_table(parquet_path)
...
def test_gate12_label_boundary_handling() -> None:
    """Gate 12: Verify label construction for H=1 and H=6 has strict end-of-sample boundary handling."""
    parquet_path = Path("data/parquet/research/EURUSD_M5_canonical.parquet")
    table = pq.read_table(parquet_path)
    T = table.num_rows
    closes = np.array([float(c) for c in table["close"].to_pylist()])
